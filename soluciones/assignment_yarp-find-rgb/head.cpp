...
protected:
    ...
    bool moveHead()
    {
        ...
        Vector encoder, command, tmp;
        encoder.resize(nAxes);
        command.resize(nAxes);
        tmp.resize(nAxes);
        int i;
        for(i = 0; i<nAxes; i++){
            tmp[i] = 10.0;
    ...
    bool findColor()
    {
        ...
        if (image!=NULL)
        {
            ...
            center_u = image ->width()/2;
            center_v = image->height()/2;
            
            
            PixelRgb &pixel = image->pixel(center_u, center_v);
            ...
            color=colorPort.prepare();
            color.addString(to_string(pixel.r) + " " + to_string(pixel.g) + to_string(pixel.b) + " ");
            printf("\n");
...
public:
   ...
   /****************************************************/
    bool getAngle()
    {
        ...
        angle_bottle = anglePort.read(false);
        ...
        {
            printf("El ángulo ha sido obtenido\n");
            // check if the bottle is correctly filled:
            // the test will send to you code the following messagge:
            // string ("angle") + double
            // how do you read it?
            angle = angle_bottle->get(0).asFloat64();
            Bottle &out = anglePort.prepare();
            out.addString("angle: " + to_string(angle));
            anglePort.setReplier(out);

            return true;
        }
        printf("donde ctm está el ángulo???\n");
