...
protected:
    ...
    bool getCOG(ImageOf<PixelRgb> &img, Vector &cog) 
    {
        ...
        for (int x=0; x<img.width(); x++)//ancho
        {
            for (int y=0; y<img.height(); y++)//largo
            {
    ...
    Vector retrieveTarget3D(const Vector &cogL, const Vector &cogR)//me lo salté pero es como el 6° called
    {
        Vector x;
    ...
    void fixate(const Vector &x)
    {
        ...
        igaze->setTrackingMode(true);/*In tracking mode when the controller reaches the target, 
        it keeps on running in order to maintain the gaze on target in the presence of external 
        disturbances. In non-tracking mode the controller releases the control of the head as 
        soon as the desired target is reached.*/
    }


    Vector computeHandOrientation()//7° called
    {
        Matrix R(3,3);//matriz de rotacion
    ...
    void approachTargetWithHand(const Vector &x, const Vector &o)
    {
        ...
        Vector approach_x=x;
        approach_x[1]+=0.1;//movimiento mano
        iarm->setTrajTime(0.8);
        iarm->goToPoseSync(approach_x,o);
        iarm->waitMotionDone(1.0,4.0);
    }

    void roll(const Vector &x, const Vector &o)//9°
    {
        Vector target_x=x;
        target_x[1]-=0.1;//movimiento pelota
    ...
    void look_down()
    {
        // we ask the controller to keep the vergencere
...
public:
    bool configure(ResourceFinder &rf)
    {
        ...
        while (Time::now()-t0<10.0)
        {
            ...
            // is not connected to solver yetn
    ...
    double getPeriod()//lo llama updateModule
    {
        return 0.0;     // sync upon incoming images
    }

    bool updateModule() //1° called
    {
        // get fresh images
        ...
        mtx.lock();//Locks the mutex. If another thread has already locked the mutex, a call to lock will block execution until the lock is acquired.
