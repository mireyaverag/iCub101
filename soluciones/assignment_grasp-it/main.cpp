...
protected:
    ...
    Vector computeHandOrientation(const string &hand)
    {
        ...
        Rot(0,0)= sqrt((double)3)/((double)2); Rot(0,1)= 0.0; Rot(0,2)= 0.5;
        Rot(1,0)= 0.0; Rot(1,1)= -1.0; Rot(1,2)=0.0;
        Rot(2,0)= -0.5; Rot(2,1)= 0.0; Rot(2,2)= sqrt((double)3)/((double)2);
                                ...
                                const Vector &o)
    {
        ...
        newDof[1] = 0;
        newDof[2] = 1;
        yInfo()<<"Valor " + to_string(newDof[3]);
        iarm->setDOF(newDof, curDof);
    ...
    void liftObject(const string &hand)
    {
        ...
        if (hand=="right")
            ...
            drvArmL.view(iarm);
        Vector curX(3), curO(3);
        iarm->getPose(curX, curO);

        curX[2] += 0.2;
        //iarm->setTrajTime(0.8);
        iarm->goToPoseSync(curX, curO);
        iarm->waitMotionDone(1.0, 4.0);  
                     ...
                     const double fingers_closure)
    {
        ...
        for (size_t i=0; i<joints.size(); i++)
        {
            ...
            movJ = min_j + fingers_closure_sat*(max_j-min_j);
            ipos->positionMove(j, movJ);
    ...
    bool openPosition(const string &robot, const string &hand){
        ...
        if (!ok)
        {
            yError()<<"Unable to open the Cartesian Controller for "<<hand;
            return false;
        }
        return true;
...
public:
    ...
    bool configure(ResourceFinder &rf)
    {
        ...
        optGaze.put("local", "/" +robot+"/gaze");
        
        
        if(!drvGaze.open(optGaze)){
            yError()<<"Unable to open the Gaze Controller";
            drvArmR.close();
            drvArmL.close(); //Cierra el otro módulo
            return false;
        }
